...
class Registers():
    """ Vectores en tablas
        ...
        ram         0x1c00-0x1fff
                    0x2000  SP (se inicialize automaticamente por el chip)
    ...
    # Nombres de las banderas para uso en 'dump' y para uso 'exterior'
    ...
    def get_flag(self, flag):
        """ Return True if Flags[flag] es 1
        """
        return (self.reg[self.SR] & (1 << flag)) != 0
    ...
    def dump(self):
        for quad in range(0, 16, 4):
            ...
            d += "  ".join(
                ["0x{0:04x}/{0:<5d}".format(self.reg[quad + i]) for i in range(4)])
            print(t + "\n  " + d)
...
class MSP430():
    ...
    OPCODES = (
        ...
        #   Instrucciones que no pueden llevar .B o .W (siempre Word)
    ...
    def single_sd(self, opcode):
        ...
        """
        return (opcode >> 4) & 3, opcode & 0x000f
...
class MSP430_disassembler(MSP430):
    def __init__(self, memory):
    ...
    def double_operand(self, opcode, pc):
        ...
        if smode == 0:
            ...
            elif sreg == Registers.CG2: line = "#0"
    ...
    def disassemble_one(self, pc):
        ...
        if form == "JMP":
            line = "{:8s}{}".format(mnem,
                    decode_signed(opcode & 0x03ff, 10) * 2)
...
class MSP430_emulator(MSP430):
    ...
    def get_src(self, pc, mode, reg):
        if mode == 0:
            return pc, self.registers.get_reg(reg)
    ...
    def op_rra(self, in_val, byte_op):
        ...
        if (in_val & msb) != 0:             # Valor es negativo?
            in_val = (in_val >> 1) | msb    # duplicamos el msb
        else:
    ...
    def op_push(self, in_val, byte_op):
        ...
        self.memory.ram.write_word(new_SP, in_val)
    ...
    def op_call(self):
        ...
        regs = self.registers
        new_SP = regs.get_reg(regs.SP) - 2
        self.memory.ram.write_word(new_SP, regs.get_reg(regs.PC))
        self.registers.set_reg(regs.SP, new_SP)
    ...
    def op_sxt(self, in_val):
        mask =0x0080
        #       0000 0000 1000 0000 
        #       1111 1111 0001 0001     0xff11
        #--------------------------
    ...
    def op_cmp(self, src, dst, byte_op):                # CMP --> Compare source word or byte and destination word or byte
        ...
        regs = self.registers
        mask = 0x0080 if byte_op else 0x8000
        regs.set_flag_by_name("C", (dst & mask) != 0)        #Es 1 cuando el MSB es 1
    ...
    def op_bit(self, src, dst, byte_op):                # BIT --> Bit Test
        """ Operacion BIT: Testea los bits entre src y dst.
            El Status Register se modifica.
        """
        ret = dst
    ...
    def op_add(self, src, dst, byte_op):
        """ Operacion ADD: Toma el el segundo dato y se lo suma
            al segundo como resultado.
            El Status Register no se modifica.
        """
        mask = 0x0080 if byte_op else 0x8000
        maskb = (0x00ff if byte_op else 0xffff)
        ...
        flagc= True if ((dst + src) > maskb) else False
        
        #No estaba seguro de que manera interpretar el ejercicio asique lo hice de 2 formas distintas
        #La primera es como se implemetaría de manera de que los numeros con signo binarios tengan sentido. La segunda es directamente una suma.
        #dst = ( (dst + src) & maskb ) if (flaga | flagb) else ( ( (dst - src) & maskb ) if (src>dst) else (src - dst) & maskb )
        dst = (dst + src) & maskb
    ...
    def op_dadd(self, src, dst, byte_op):
        ...
        mask = 0x0080 if byte_op else 0x8000
        # Status register update
        #regs.set_flag_by_name("C", False)                  #que es bcd??
        #regs.set_flag_by_name("V", False)                  #que es indefinido?
        #regs.set_flag_by_name("Z", dst == 0 )
        #regs.set_flag_by_name("N", (dst & mask) != 0)  
    ...
    def single_step(self):
        ...
        if form == "JMP":
            ...
            if mnem == "JMP":
                ...
                jump = not regs.get_flag(regs.get_flag_by_name("Z"))
            elif mnem == "JZ":
            ...
            if jump:
                offs = decode_signed(opcode & (mask ^ 0xffff), 10)
                pc = pc + offs * 2
